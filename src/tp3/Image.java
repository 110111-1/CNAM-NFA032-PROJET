...
public class Image {
	...
	//TP3
	public void incrusterCoinGaucheVoid(Image avPlan){//TP3 Question 1 methode void modifier l'image
		for (int col = 0; col < avPlan.getTabPixel().length; col++) {
			for (int ligne = 0; ligne < avPlan.getTabPixel()[0].length; ligne++) {
				if(avPlan.tabPixel[col][ligne].getAlpha()==0){
					avPlan.tabPixel[col][ligne] = tabPixel[col][ligne];
				}
				else{
	...
	public static void main(String[] args) {
		Image prairie = new Image("images/extrait-prairie.png");
		Image renard = new Image ("images/renard.png");

		//Question 1
		prairie.incrusterCoinGauche(renard).afficheImage();

		//Question 2
		Image perruche = new Image("images/perruche.png");
		perruche.incrusterCarre().afficheImage();

		//Question 3
		prairie.incruster(renard,125,60).afficheImage();
		prairie.incruster(renard,400,220).afficheImage();
		prairie.incruster(renard, -200, 30).afficheImage();
	}
} 